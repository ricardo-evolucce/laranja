...
try {
 ...
 try {
        $ativo = '1';
        $stmt = $conexao->prepare("INSERT INTO aluno (nome, email, aceitaNewsletter) VALUES (?, ?, ?)");
        $stmt->bindParam(1, $nome);
        $stmt->bindParam(2, $email);

         $stmt->bindParam(3, $aceitaNewsletter);
        ...
        if ($stmt->execute()) {
            if ($stmt->rowCount() > 0) {
                ...
                echo "<script>alert('Curso cadastrado com sucesso');</script>";
  	             echo "<script>location.href='adiciona-inscrito-news.php';</script>";
